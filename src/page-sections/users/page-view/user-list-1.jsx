...
export default function UserList1PageView() {
  ...
  const [customerPics, setCustomerPics] = useState([]); // Fetch customers from Supabase
  ...
  const fetchCustomerPics = async () => {
    const { data, error } = await supabase.from("customer_pics").select("*, customers(nama_pt)");
    if (!error) setCustomerPics(data);
    setUsers(data);
  };
  ...
  const { page, order, orderBy, selected, isSelected, rowsPerPage, setPage, handleSelectRow, handleChangePage, handleRequestSort, handleSelectAllRows, handleChangeRowsPerPage } = useMuiTable({
    defaultOrderBy: "name",
  });
  useEffect(() => {
    setPage(0);
  ...
  const handleChangeFilter = useCallback((key, value) => {
    setUserFilter((prevFilter) => ({ ...prevFilter, [key]: value }));
  }, []);
            ...
            <Table>
              ...
              <TableBody>
                {paginatedUsers.length === 0 ? (
                  ...
                  paginatedUsers.map((user) => <UserTableRow key={user.id} user={user} isSelected={isSelected(user.id)} handleSelectRow={handleSelectRow} handleDeleteUser={handleDeleteUser} />)
                )}
              </TableBody>
            </Table>
          </Scrollbar>
