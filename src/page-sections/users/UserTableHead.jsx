...
export default function UserTableHead({ order, orderBy, rowCount, numSelected, onRequestSort, onSelectAllRows }) {
    ...
    <StyledTableHead>
      <TableRow>
        ...
        {headCells.map((headCell) => (
          <HeaderCell key={headCell.id} padding={headCell.disablePadding ? "none" : "normal"} sortDirection={orderBy === headCell.id ? order : false}>
            <TableSortLabel active={orderBy === headCell.id} onClick={createSortHandler(headCell.id)} direction={orderBy === headCell.id ? order : "asc"}>
              {headCell.label}
              {orderBy === headCell.id ? <span style={visuallyHidden}>{order === "desc" ? "sorted descending" : "sorted ascending"}</span> : null}
            </TableSortLabel>
          </HeaderCell>
        ))}
      </TableRow>
